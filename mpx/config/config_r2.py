...
body_name = ['Lankle-roll','Rankle-roll']
...
p0 = jnp.array([0, 0, 0.64])  # Initial position of the robot's base
...
p_legs0 = jnp.array([ 0.12592681,  0.135, 0.01690434,
                      0.12592681,  0.015, 0.01690434,
                     -0.10407319,  0.135, 0.01690434,
                     -0.10407319,  0.015, 0.01690434,
                      0.12592681, -0.015, 0.01690434,
                      0.12592681, -0.135, 0.01690434,
                     -0.10407319, -0.015, 0.01690434,
                     -0.10407319, -0.135, 0.01690434 ])

# Determine number of joints and contacts from the lists
n_joints = 31  # 关节控制自由度
n_contact = len(contact_frame)  # 接触点数量
grf_as_state = False
# 状态数量: p(3) + quat(4) + q(31) + dp(3) + omega(3) + dq(31) + foot_pos(8*3)
if grf_as_state:
    n = 13 + 2*n_joints + 3*n_contact + 3*n_contact  # 包含GRF: 123维
else:
    n = 13 + 2*n_joints + 3*n_contact  # 不包含GRF: 99维
...
Qleg = jnp.diag(jnp.tile(jnp.array([1e3,1e5,1e5]),n_contact))
