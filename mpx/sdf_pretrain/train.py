...
Usage:
    ...
    python -m mpx.sdf_pretrain.train --use-curriculum --regenerate-every 50
"""
...
def create_train_state(rng, model, learning_rate, input_shape, query_shape):
    ...
    state = TrainState.create(
        ...
        metrics=Metrics(mse=0.0, eikonal=0.0, total=0.0),
    )
    return state
...
@partial(jax.jit, static_argnames=['compute_eikonal'])
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if args.regenerate_every > 0 and epoch > 0 and epoch % args.regenerate_every == 0:
            key, subkey = jax.random.split(key)
            print(f"Regenerating terrain for epoch {epoch+1}...")
            generator.regenerate_terrain(subkey)
        ...
        if np.isnan(mse_val) or np.isnan(eik_val):
            ...
            print(f"  MSE: {mse_val}")
            print(f"  Eikonal: {eik_val}")
            print("  Skipping this epoch and regenerating terrain...")
            key, subkey = jax.random.split(key)
            generator.regenerate_terrain(subkey)
            continue
